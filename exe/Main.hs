...
import StackageToHackage.Hpack (execHpack, hpackInput)
import StackageToHackage.Stackage (localDirs, readStack)

import Control.Exception (throwIO)
import Control.Monad (filterM, when)
import Data.Dates.Parsing (defaultConfigIO, parseDateTime)
import Data.Foldable (traverse_)
import Data.Hourglass (Elapsed, timeConvert)
    ...
    Opts
        ...
        <*> optional
                (strOption
                    ...
                    <> metavar "FUZZY_DATE"
                    <> help
                           "Pin hackage index state (values like \"now\" and \"yesterday\" work)"
                    )
                )
...
main = do
    let
        versionHelp =
            infoOption version (long "version" <> help "Show version" <> hidden)

    customExecParser
            (prefs showHelpOnError)
            (info (optsP <**> helper <**> versionHelp) idm)
        >>= \Opts {..} -> do
        -- read stack file
                ...
                when runHpack $ do
                    hpacks <- filterM (doesFileExist . hpackInput) subs
                ...
                case (hackageIndexDate, dt) of
                    (Just d, Nothing) -> throwIO $ userError
                        ("Warning: failed to convert hackage index state date \""
                        <> d
                        <> "\""
                        )
                    _ -> pure ()
                (project, freeze) <- stackToCabal
                    inspectRemotes
                    runHpack
                    inDir
                    stack
...
extractHack (T.split ('\n' ==) -> lines) =
    let
        (_, region) = break (T.isPrefixOf "#+BEGIN_STACK2CABAL") lines
