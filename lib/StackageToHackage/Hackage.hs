...
import StackageToHackage.Hackage.Types (Constraint(..), Freeze(..), Project(..))
import StackageToHackage.Hpack (execHpack, hpackInput)
import StackageToHackage.Stackage (localDirs, mergeResolvers, unroll)
...
import Data.Hourglass (Elapsed, ISO8601_DateAndTime(..), timePrint)
...
printFreeze (Freeze constraints) = T.concat
  ...
  where
    ...
    printConstraints =
        T.intercalate spacing . fmap printConstraint $ constraints

    printConstraint :: Constraint -> Text
    printConstraint (VersionPin pkg) =
        let
            name = (T.pack . unPackageName . pkgName $ pkg)
...
genFreeze Resolver { deps, flags } ignore =
    let
        pkgs = filter noSelfs $ unPkgId <$> mapMaybe pick deps
    ...
    = withSystemTempDirectory "stack2cabal" $ \dir ->
        handleIOError
                (\_ ->
                    hPutStrLn
                            stderr
                            ("Warning: failed to resolve remote .cabal files of: "
                            <> show git
                            )
                        >> pure []
                )
...
getPackageIdent dir =
    handleIOError
            (\_ ->
                hPutStrLn
                        stderr
                        ("Warning: failed to resolve .cabal file in " <> dir)
                    >> pure Nothing
            )
        $ do
              cabalFile <- headMay <$> getDirectoryFiles dir ["*.cabal"]
              forM cabalFile $ \f ->
                  package . packageDescription <$> readGenericPackageDescription
                      silent
                      (dir </> f)
...
getRemotePkgs srcs runHpack =
    fmap concat $ forM srcs $ \src -> getRemotePkg src runHpack
